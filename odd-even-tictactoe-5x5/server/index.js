 ...
 *   clients: Map<WebSocket, { id, name, role: 'odd'|'even'|'spectator', joinedAt }>
 * }
 */
const rooms = new Map();
...
function publicState(room) {
  ...
  return {
    ...
    winner: room.winner, // 'odd' | 'even' | null
    players,
    spectatorCount
  };
}
...
function tryPromoteSpectator(room, roleToFill) {
  // Choose earliest-joined spectator
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'increment') {
      ...
      if (me.role !== 'odd' && me.role !== 'even') return; // spectators cannot play
      const { row, col } = msg;
      if (
    ...
    if (msg.type === 'restart') {
      // Allow any player to restart; spectators cannot.
  ...
  ws.on('close', () => {
    // Cleanup and possible role promotion
    const r = rooms.get(roomId);
    if (!r) return;
